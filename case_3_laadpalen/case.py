...
rdw.rename(
    {
        "datum_eerste_toelating": "date_of_acceptance",
        "benzine_count": "gasoline_count",
        "elektriciteit_count": "electric_count",
        "waterstof_count": "hydrogen_count",
    },
    inplace=True,
    axis=1,
)
...
response = requests.get(
    "https://api.openchargemap.io/v3/poi/?output=json&countrycode=NL&maxresults=10000&compact=true&verbose=false&key=772ba88b-50db-49f1-b9f1-933c6053edd3"
)
...
def folium_static(
    ...
    width=700,
    height=500,
    colors=["#5e4fa2", "#89d0a4", "#fffebe", "#f88c51", "#9e0142"],
...
for column in columns:
    cum_fuel.add_trace(
        go.Scatter(
            x=rdw["date_of_acceptance"],
            y=rdw[column],
            name=column.split("_")[0].title(),
        )
    )
...
cum_fuel_dropdown = [
    {
        ...
        "args": [
            ...
            {"title": "Cumulative sum of vehicles per fuel type over time"},
        ],
    },
    {
        "label": "Gasoline",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of gasoline vehicles over time"},
        ],
    },
    {
        "label": "Electric",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of electric vehicles over time"},
        ],
    },
    {
        "label": "Diesel",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of diesel vehicles over time"},
        ],
    },
    {
        "label": "Hydrogen",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of hydrogen vehicles over time"},
        ],
    },
    {
        "label": "Alcohol",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of alcohol vehicles over time"},
        ],
    },
    {
        "label": "LPG",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of LPG vehicles over time"},
        ],
    },
    {
        "label": "CNG",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of CNG vehicles over time"},
        ],
    },
    {
        "label": "LNG",
        ...
        "args": [
            ...
            {"title": "Cumulative sum of LNG vehicles over time"},
        ],
    },
]

cum_fuel.update_layout(
    legend={"title": "Fuel type"},
    title={"text": "Cumulative sum of vehicles per fuel type over time"},
...
with col1:
    ...
    rdw["electric_percentage"] = (
        rdw["electric"] / (rdw["electric"] + rdw["non_electric"]) * 100
    )
    ...
    df["percentage"] = (
        df["pre_electric"] / (df["pre_electric"] + df["pre_non_electric"]) * 100
    )
    df["date"] = X_predict2
    prediction_plot = px.line(
        x=df.date,
        y=df.percentage,
        title="Predicted growth of the percentage of electric vehicles over time.",
        labels={"y": "Percentage of electric vehicles", "x": "Date"},
    )

    st.markdown(
        "Below is the predictive plot of the percentage of electric vehicles in relation to all vehicles. This shows that over time, it is expected that electric vehicles will become the large majority of vehicles. This has been deduced using a linear regression algorythm from Scikit learn."
    )
...
with col2:
    ...
    for i, row in ocm.iterrows():
        folium.Circle(
            ...
            radius=60 * zoom,
            popup="<b>Addres:</b> "
            + row["addressinfo_title"]
            + ",\n<b>Power output:</b> "
            + str(row["powerkw"])
            + " kW,\n<b>Number of charging points:</b> "
            + str(round(row["numberofpoints"])),
    ...
    fig = ff.create_distplot(
        hist_data, group_labels, bin_size=[0.1, 0.25, 0.5, 1], show_rug=False
    )
    fig.update_layout(
        {"title": {"text": "Probability distribution of charging & connected time"}},
